...
class PinCenterController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        $requestedPins = DB::select('select `users`.*, `request_pin`.*, `request_pin`.`created_at` as `req_created_at`,"inpin" AS requesttype from `request_pin` left join `users` on `users`.`user_slug` = `request_pin`.`admin_slug` where `request_pin`.`req_user_id` = '.Auth::user()->id.' AND `request_pin`.`status` = "Pending" UNION select `users`.*, `request_pin`.*, `request_pin`.`created_at` as `req_created_at`,"outpin" AS requesttype from `request_pin` left join `users` on `users`.`id` = `request_pin`.`req_user_id` where `request_pin`.`admin_slug` = "'.Auth::user()->user_slug.'"  AND `request_pin`.`status` = "Pending"');
