  ...
  <div class="auth-inner py-2">
    ...
    <div class="card mb-0">
      <div class="card-body">
        ...
        <form class="auth-reset-password-form mt-2" method="POST" action="{{ route('password.update') }}" id="jquery-val-form">
          @csrf
          <input type="hidden" name="token" value="{{ $token }}">

          <div class="form-group">
            {{-- <label for="email" class="form-label">Email</label> --}}
            <input type="hidden" class="form-control @error('email') is-invalid @enderror" id="email" name="email" placeholder="Email" aria-describedby="email" tabindex="1" autofocus value="{{ $email ?? old('email') }}" />
              @error('email')
