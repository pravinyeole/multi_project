...
$(function () {
  ...
  if (addTaskBtn.length) {
    addTaskBtn.on('click', function (e) {
      ...
      modalTitle.text('Update Error Details');
    ...
    newTaskForm.on('submit', function (e) {
      ...
      if (isValid) {
        ...
        if (todoTitle != '') {
          ...
          //   checkboxId +'" />' +
          //   '<label class="custom-control-label" for="customCheck' + checkboxId +
          //   '"></label>' +
          //   '</div>' +
          //   '<span class="todo-title">' + todoTitle +
          //   '</span>' +
          //   '</div>' +
          //   '</div>' +
          //   '<div class="todo-item-action">' +
          //   '<div class="badge-wrapper mr-1">' + todoBadge +
          //   '</div>' +
          //   '<small class="text-nowrap text-muted mr-1">' + todoDate +
          //   '</small>' +
          //   '<div class="avatar">' +
          //   '<img src="' +  membersImg[assignedTo] +
          //   '" alt="' +assignedTo +
          //   '" height="28" width="28">' +
          //   '</div>' +
          //   '</div>' +
  ...
  $(document).on('click', '.apply', function (e) {
    ...
    var arr = [];
    $('.customCheck1:checked').each(function(){
       arr.push($(this).data('resid'));
    });
    if (arr.length === 0) {
      newTaskModal.modal('hide');
      ...
      if(arr.length == 1){
        $("#result_description").text(arr.length+' record selected');
      }else{
        $("#result_description").text(arr.length+' records selected');
      }
  ...
  $(document).on('click', '.todo-task-list-wrapper .todo-item', function (e) {
    ...
    if (arr.length > 0) {
      bootbox.alert("Please click on Bulk Edit button to update details");
      e.stopPropagation();

      return;
    }
    var keyid = $(this).data('id');
    var id = $('#resultId' + keyid).val();
    var url = $('#url').val();
    ...
    $.ajax({
        ...
        success: function (result) {
          ...
          newTaskModal.modal('show');
          var result_description = result.data.result_description;
          var ssn = result.data.ssn;
          if(ssn!=null){
            const ssnIdlength = ssn.length;
            const maskedLength = ssnIdlength - 4; /** Modify the length as per your wish */
            let newSSN = ssn;
            for (let i = 0; i < ssnIdlength; i++) {
              if (i < maskedLength) {
                newSSN = newSSN.replace(ssn[i], '*');
              }
            }
             result_description = result_description.replace('{SSN}', newSSN)
          }
         
          $("#result_description").html(result_description);

          $("#status").empty('');
          $("#status").append('<option value=""> Select Status</option>')
          if (result.data.status == "Open") {
            $("#status").append('<option value="Open" selected>Open</option>');
            $("#status").append('<option value="In-Progress">In Progress </option>');
            // $("#status").append('<option value="Resolved">Resolved</option>');
            $("#status").append('<option value="Closed" >Completed</option>');
          } else if(result.data.status == "Closed"){
            $("#status").append('<option value="Open" >Open</option>');
            $("#status").append('<option value="In-Progress">In Progress </option>');
            // $("#status").append('<option value="Resolved">Resolved</option>');
            $("#status").append('<option value="Closed" selected>Completed</option>');
          // } else if(result.data.status == "Resolved"){
          //   $("#status").append('<option value="Open" >Open</option>');
          //   $("#status").append('<option value="In-Progress">In Progress </option>');
          //   $("#status").append('<option value="Resolved" selected>Resolved</option>');
          //   $("#status").append('<option value="Closed" >Completed</option>');
          // }
          }else{
            $("#status").append('<option value="Open" >Open</option>');
            $("#status").append('<option value="In-Progress" selected>In Progress </option>');
            // $("#status").append('<option value="Resolved" >Resolved</option>');
            $("#status").append('<option value="Closed" >Completed</option>');
          }
          ...
          if (result.data.priority == "Low") {
            $("#priority").append('<option value="Low" selected>Low</option>');
            $("#priority").append('<option value="Medium">Medium </option>');
            $("#priority").append('<option value="High">High</option>');
          } else if(result.data.priority == "Medium"){
            $("#priority").append('<option value="Low" >Low</option>');
            $("#priority").append('<option value="Medium" selected>Medium </option>');
            $("#priority").append('<option value="High">High</option>');
          } else if(result.data.priority == "High"){
            $("#priority").append('<option value="Low" >Low</option>');
            $("#priority").append('<option value="Medium">Medium </option>');
            $("#priority").append('<option value="High" selected>High</option>');
          }else {
            $("#priority").append('<option value="Low" >Low</option>');
            $("#priority").append('<option value="Medium">Medium </option>');
            $("#priority").append('<option value="High" >High</option>');
          ...
          $("#team").empty('');
          $("#team").append('<option value=""> Select Team Member</option>');
          for(let i=0;i<result.users.length;i++){
            if(result.users[i].id == result.data.assign_team_id ){
              $("#team").append('<option value='+result.users[i].id+' '+"selected"+'>'+result.users[i].user_fname+" "+result.users[i].user_lname+'</option>');
            }
            else{ 
              $("#team").append('<option value='+result.users[i].id+'  >'+result.users[i].user_fname+" "+result.users[i].user_lname+'</option>');
            }
          }


          // $("#team").empty('');
          // $("#team").append('<option value=""> Select Team</option>')
          // if (result.data.priority == "Low") {
          //   $("#team").append('<option value="Low" selected>Low</option>');
          //   $("#team").append('<option value="Medium">Medium </option>');
          //   $("#team").append('<option value="High">High</option>');
          // } else if(result.data.priority == "Medium"){
          //   $("#team").append('<option value="Low" >Low</option>');
          //   $("#team").append('<option value="Medium" selected>Medium </option>');
          //   $("#team").append('<option value="High">High</option>');
          // } else if(result.data.priority == "High"){
          //   $("#team").append('<option value="Low" >Low</option>');
          //   $("#team").append('<option value="Medium">Medium </option>');
          ...
          $("#description").val(result.data.note);
          $("#resultupdateId").val(result.data.audit_result_details_id);
        }
    });
    ...
    var quill_editor = $('#description .ql-editor'); // ? Dummy data as not connected with API or anything else
    ...
    // taskTitle = $(this).find('.error_status');
    // var $title = $(this).find('.error_status').html();

    // apply all variable values to fields
    // newTaskForm.find('.error_status').val($title);
  });
